...
export const authService = {
  async login(credentials: LoginCredentials): Promise<ApiResponse<User>> {
    try {
      ...
      const approvedUser = approvedUsers.find((u: any) => 
        u.accountNumber === credentials.accountNumber && u.pin === credentials.pin
      );

      if (approvedUser) {
        // Send security SMS for approved users
        try {
          await smsService.sendSecuritySMS(approvedUser.id, approvedUser.phone || '01712345678', 'login', {
            date: new Date().toLocaleDateString('bn-BD'),
            time: new Date().toLocaleTimeString('bn-BD')
          });
        } catch (smsError) {
          const appError = handleError(smsError, 'AuthService');
          log.error('Failed to send login security SMS', appError, 'AuthService');
        }
        
        const user: User = {
          id: approvedUser.id,
          name: approvedUser.name,
          firstName: approvedUser.firstName || approvedUser.name.split(' ')[0],
          lastName: approvedUser.lastName || approvedUser.name.split(' ')[1] || '',
          email: approvedUser.email,
          accountNumber: approvedUser.accountNumber,
          balance: approvedUser.balance || 0,
          phone: approvedUser.phone,
          address: approvedUser.address,
          nationality: approvedUser.nationality || 'Bangladeshi',
          notificationPreferences: approvedUser.notificationPreferences || {
            smsEnabled: true,
            emailEnabled: true,
            transactionAlerts: true,
            securityAlerts: true,
      ...
      if (user) {
        ...
        if (attemptedUser) {
          try {
            ...
            const appError = handleError(smsError, 'AuthService');
            log.error('Failed to send failed login security SMS', appError, 'AuthService');
          }
        }
