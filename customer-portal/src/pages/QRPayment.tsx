...
import { handleError, handleTransactionError } from '../utils/errorHandler';
...
const QRPayment: React.FC = () => {
  ...
  const fetchCurrentBalance = async () => {
    try {
      ...
      const appError = handleError(error, 'QRPayment');
      log.error('Error fetching balance', appError, 'QRPayment');
      showError('Failed to fetch account balance. Please try again.');
      setLoadingBalance(false);
    }
  };
    ...
    try {
      if (videoRef.current) {
        const scanner = new QrScanner(
          ...
          (result) => {
            try {
              const qrPaymentData = JSON.parse(result.data);
              setQrData(qrPaymentData);
              setFormData(prev => ({
                ...prev,
                amount: qrPaymentData.amount?.toString() || '',
                merchantName: qrPaymentData.merchantName || '',
                merchantId: qrPaymentData.merchantId || '',
      ...
      const appError = handleError(error, 'QRPayment');
      log.error('Error accessing camera', appError, 'QRPayment');
  ...
  const handleConfirmTransaction = async () => {
    try {
      ...
      const appError = handleTransactionError(error, 'QRPayment');
      log.error('Transaction error', appError, 'QRPayment');
            ...
            <div className="space-y-6">
              ...
              {qrData && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                  <h3 className="font-semibold text-blue-900 mb-4">QR Code Detected - Confirm Payment</h3>
                  <div className="space-y-3 text-sm mb-4">
                    <div className="flex justify-between">
                      <span className="font-medium text-gray-700">Merchant:</span> 
                      <span className="text-gray-900">{qrData.merchantName}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-medium text-gray-700">Amount:</span> 
                      <span className="text-gray-900 font-semibold">৳{qrData.amount}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-medium text-gray-700">Description:</span> 
                      <span className="text-gray-900">{qrData.description}</span>
                    </div>
                  </div>
              ...
              {generatedQRCode && (
                <div className="mt-6 text-center">
                  ...
                  <p className="mt-4 text-sm text-gray-600">
                    Share this QR code with customers to receive payments
                  </p>
                  <div className="mt-4 flex justify-center space-x-4">
                    <button
                      onClick={() => {
                        const link = document.createElement('a');
                        link.download = `qr-code-${formData.merchantName || 'payment'}.png`;
