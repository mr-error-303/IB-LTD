...
const Withdraw: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBalance = async () => {
      try {
        ...
        const appError = handleError(error, 'Withdraw');
        log.error('Error fetching balance', appError, 'Withdraw');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (withdrawAmount > currentBalance) {
      setError(t(`Insufficient funds. Your current balance is $${currentBalance.toFixed(2)}`));
      setLoading(false);
      return;
    }

    if (withdrawAmount > 10000) {
      setError(t('Maximum withdrawal amount is $10,000 per transaction'));
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setSuccess(t(`Successfully withdrew $${formData.amount}. New balance: $${newBalance.toFixed(2)}`));
      setFormData({ amount: '', description: '' });
    } catch (error) {
      const appError = handleError(error, 'Withdraw');
      log.error('Withdrawal error', appError, 'Withdraw');
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-md mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-gray-900 mb-2">{t('Withdraw Money')}</h1>
            <p className="text-gray-600">{t('Withdraw funds from your account')}</p>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center pt-2 border-t">
                <span className="text-gray-600">{t('Available Balance')}:</span>
                {loadingBalance ? (
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <div className="mt-1 flex justify-between text-xs text-gray-500">
                  <span>{t('Maximum: $10,000 per transaction')}</span>
                  <span>{t('Available')}: ${currentBalance.toFixed(2)}</span>
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={t('e.g., ATM withdrawal, Cash needed')}
                />
              </div>
            ...
            <div className="mt-6 p-4 bg-yellow-50 rounded-md">
              <div className="flex">
                ...
                <div>
                  <h3 className="text-sm font-medium text-yellow-800">{t('Important Notice')}</h3>
                  <p className="text-sm text-yellow-700 mt-1">
                    {t('Withdrawals are processed immediately and cannot be reversed. Please verify the amount before confirming.')}
                  </p>
                </div>
              </div>
            </div>
